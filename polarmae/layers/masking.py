...
class MaskedLayerNorm(torch.nn.Module):
    ...
    def forward(self, tensor: torch.Tensor, mask: torch.BoolTensor) -> torch.Tensor:
        ...
        return (
            self.gamma * (tensor - mean) / (std + tiny_value_of_dtype(tensor.dtype)) + self.beta
        ) * broadcast_mask
...
class MaskedBatchNorm1d(nn.Module):
    ...
    def forward(self, x, mask=None):
        ...
        # Normalize
        x.div_(torch.sqrt(var + self.eps).view(1, C, 1))
        x.mul_(mask)  # Multiply by mask to zero out padded positions
