...
class PointcloudCenterAndNormalize(nn.Module):
    def __init__(
        self,
        center: torch.Tensor | List[float] | float,
        scale_factor: torch.Tensor | List[float] | float,
    ):
        super().__init__()
        self.center = center
    ...
    def forward(self, points: torch.Tensor):
        ...
        if isinstance(self.center, list):
            self.center = torch.tensor(self.center, device=points.device, dtype=points.dtype)
        elif isinstance(self.center, torch.Tensor) and self.center.device != points.device:
            self.center = self.center.to(points.device).to(points.dtype)

        if isinstance(self.scale_factor, list):
            self.scale_factor = torch.tensor(self.scale_factor, device=points.device, dtype=points.dtype)
        elif isinstance(self.scale_factor, torch.Tensor) and self.scale_factor.device != points.device:
            self.scale_factor = self.scale_factor.to(points.device).to(points.dtype)

        points[:, :, :3] = (points[:, :, :3] - self.center) * self.scale_factor
        return points
