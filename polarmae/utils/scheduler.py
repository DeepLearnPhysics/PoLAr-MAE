...
class ValueScheduler:
    def __init__(self, initial_value, verbose=True):
...
class LinearScheduler(ValueScheduler):
    def __init__(self, start_value, end_value, total_steps, turn_on_at_step=0, verbose=True):
        super().__init__(start_value)
        self.end_value = end_value
        self.total_steps = total_steps
        self.delta = (end_value - start_value) / total_steps
        self.verbose = verbose
        self.turn_on_at_step = turn_on_at_step

    def step(self):
        if self.step_count < self.turn_on_at_step:
            self.value = self.initial_value
        elif self.step_count < self.total_steps+self.turn_on_at_step:
            self.value += self.delta
        else:
            self.value = self.end_value
        self.step_count += 1
        if self.verbose:
            print(f"Voxel size: {self.value}")


class CosineScheduler(ValueScheduler):
    def __init__(self, start_value, end_value, total_steps, verbose=True):
        super().__init__(start_value)
        self.end_value = end_value
        self.total_steps = total_steps
        self.verbose = verbose
...
class ConstantScheduler(ValueScheduler):
    def __init__(self, constant_value, verbose=True ):
...
# stolen from pl_bolts: https://github.com/Lightning-Universe/lightning-bolts/blob/2c4602aa684e7b90e7ffdcea1d3f93a20f9c2ead/src/pl_bolts/optimizers/lr_scheduler.py#L12
...
class LinearWarmupLR(_LRScheduler):
    ...
    .. warning::
        It is recommended to call :func:`.step()` for :class:`LinearWarmupLR`
        after each iteration as calling it after each epoch will keep the starting lr at
        warmup_start_lr for the first epoch which is 0 in most cases.
    ...
    Example:
        ...
        >>> layer = nn.Linear(10, 1)
        >>> optimizer = Adam(layer.parameters(), lr=0.02)
        >>> scheduler = LinearWarmupLR(optimizer, warmup_epochs=10)
        >>> # the default case
        >>> for epoch in range(40):
        ...     # train(...)
        ...     # validate(...)
